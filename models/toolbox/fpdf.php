...
function CellWrap($w, $h=0, &$fpdf, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='') {
    $max_length_before_wrap = $w;

	if ($fpdf->GetStringWidth($txt) > $max_length_before_wrap) {
        ...
        while ($word < $total_words and $fpdf->GetStringWidth($line . $words[0] . " ") < $max_length_before_wrap){
            $word++;
		...
		$fpdf->Cell($w, $h, $line, $border, $ln, $align, $fill, $link);
        $fpdf->Ln(5);
		//$pdf->CellWrap(20,5,$pdf,$captured_date,1,0,'L');

        // Continue to wrap the remaining text
        $rest_of_text = implode(' ', $words);
        //CellWrap($fpdf, $rest_of_text);
		$fpdf->CellWrap($w, $h, $fpdf, $rest_of_text, $border, 2, $align, $fill, $link);
    } else {
        //$fpdf->Cell(0,5,$txt, 0, 2, 'C');
		$fpdf->Cell($w, $h, $txt, $border, 0, $align, $fill, $link);
    }
}
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			...
			$nl++;
			if($border && $nl==2) {
				$b2='LR'; // added to prevent invalid variable error
				$b=$b2;
			}
			continue;
		}
...
function Output($name='', $dest='')
{
	...
	$name = str_replace('/', '_', $name); // replace invalid characters
	$name = str_replace(' ', '_', $name); // replace spaces
	$name = strtolower($name); // convert file name to lowercase
	$name = $name . '.pdf';
	if($dest=='')
	{
		if($name=='')
		...
		$dest='I'; //force saving to local file
	}
	switch($dest)
	{
		case 'I':
